...
def consolidate_listing(args,a_year=None):
    """
    pre requisite : raw_listing_edition() already run
    :param args:
    :param a_year: str YYYY
    :return:
    """
    input_listing = os.path.join(args.outputdir,'listing_SAR_L2_L1_measu_from_colocations_cwaveV4.txt')
    if a_year is None:
        output_listing  = os.path.join(args.outputdir,'listing_SAR_L2_L1_measu_from_colocations_cwaveV4_consolidated.txt')
    else:
        output_listing  = os.path.join(args.outputdir,'listing_SAR_L2_L1_measu_from_colocations_cwaveV4_consolidated_%s.txt'%a_year)
    fid =  open(input_listing,'r')
    fud = open(output_listing,'w')
    cpt_keep = 0
    cpt_tot = 0
    for lines in fid.readlines():
        if a_year is None:
            ...
            if os.path.basename(lines).split('-')[4][0:4]!=a_year:
                continue #je ne fais rien
            else:
                pass #j ajoute la ligne dans le listing
    ...
    fud.close()
    fid.close()
    logging.info('done -> %s',output_listing)
if __name__ == '__main__':
    ...
    # possible outputdir (the one used so far= /home1/datawork/agrouaze/data/sentinel1/cwave/)
