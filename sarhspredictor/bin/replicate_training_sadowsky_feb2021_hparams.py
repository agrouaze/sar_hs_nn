...
def define_model (units=256,dropout=0.5) :
    # Low-level features.
    inputs = Input(shape=(72,60,2))
    x = Conv2D(units/4,(3,3),activation='relu')(inputs)
    x = MaxPooling2D(pool_size=(2,2))(x)

    x = Conv2D(units/2,(3,3),activation='relu')(x)
    x = MaxPooling2D(pool_size=(2,2))(x)
    ...
    x = Dense(units=units,activation='relu')(inp)
    ...
    x = Dense(units,activation="relu")(combinedInput)
    x = Dropout(dropout)(x)
    x = Dense(units,activation="relu",name='penultimate')(x)
    x = Dropout(dropout)(x)
...
def start_training_regression(desst,filename_out,logdir_tensorboard,epochs,hparams):
    """
    ...
    :return:
    """
    model = define_model(hparams[HP_NUM_UNITS],hparams[HP_DROPOUT])

    initial_learning_rate = 0.0003 #paper Quach 2020 , different from 0.0001 proposed in notebooks shared by P. Sadowdski
    model.compile(loss=[Gaussian_NLL],optimizer=Adam(lr=hparams[HP_LEARNING_RATE]),metrics=[Gaussian_MSE,MAE_metric,MAPE_metric,
                                                                       COSI_SIMI,MSE_metric])
    # i dont know if i can put root_mean_squared_error as a metric or a second loss -> TBC

    # Dataset
    batch_size = hparams[HP_BATCH_SIZE] #paper Quach 2020
    ...
    hparams_clk = hp.KerasCallback(logdir_tensorboard,hparams)  # log hparams
    clbks = [reduce_lr, check, stop,tensorBoard,hparams_clk] #tensorBoard out because I suspect it slow down the process
...
if __name__ =='__main__':
    ...
    parser.add_argument('--outputModelFile',
    ...
    #je crois quil faut preparer le rep de log hparams
    results_path = '/home1/datahome/agrouaze/sources/sentinel1/hs_total/validation_quach2020/test_17sept2021_hparams'
    results_path = '/home1/datahome/agrouaze/sources/sentinel1/hs_total/validation_quach2020/test_21sept2021_hparams'
    with tf.summary.create_file_writer(os.path.join(results_path,"hparam_tuning")).as_default() :
        hp.hparams_config(
            hparams=[HP_NUM_UNITS,HP_DROPOUT,HP_LEARNING_RATE,HP_BATCH_SIZE],
    ...
    for learning_rate in HP_LEARNING_RATE.domain.values:
        for batch_size in HP_BATCH_SIZE.domain.values:
            for dropoout in HP_DROPOUT.domain.values:
                for uniits in HP_NUM_UNITS.domain.values:
                    ...
                    if args.zay:
                        ...
                        filename_out = os.path.join(results_path,'heteroskedastic_2017_agrouaze_exp0_year2017_corrected.session%s.epoch{epoch:02d}-loss{val_loss:.2f}.hdf5'%session_num)
                        filename_out = os.path.join(results_path,
                                                    'heteroskedastic_2017_agrouaze_exp0_year2017_corrected.session%s.epoch{epoch:02d}-loss{val_loss:.2f}.hdf5' % session_num)
                    print(filename_out)
                    if not os.path.exists(results_path):
                        os.makedirs(results_path,0o0755)
                    ...
                    if args.zay:
                        ...
                        logdir_tensorboard = os.path.join('/home1/scratch/agrouaze/training_quach_redo_model/exp0_year2017_corrected_hparams/')
                    t0 = time.time()
                    if args.zay: #avec la correction sur year 2017, je vois qu on peut encore gagner sur le loss du coup je met plus d epochs
                        ...
                        epochs = 123
                        epochs = 2 # just a test to see if I can get the hparams
                        #epochs = 300
                    ...
                    best_mse,best_loss,best_mae,best_mape,best_cos = start_training_regression(desst,filename_out,
                                                            logdir_tensorboard=logdir_tensorboard,epochs=epochs,hparams=hparams)
                    elaps = (time.time()-t0)/60.
                    logging.info('done: elapsed time %1.3f min',elaps)
    logging.info('end of the tests')
